...
原位文字和多档马赛克。当前开发树已接入可选 OCR，但不属于已发布 v0.3.0；UIA 子控件识别与翻译仍后置。相关组件职责、
数据流及降级条目为后续约束，具体状态见 [实现进度](implementation_progress.md)。

已发布版本为 `0.3.0`，当前源码版本为 `0.4.0`；真实登录、自启禁用、跨会话、跨 DPI 及部分视觉验收仍待完成；
...
将文档传给 Renderer 解析，再按控件 ID 注册 BindString、BindBool、BindOptions、BindAction。布局不包含业务设置键，
...
原生控件支持页面标签、纵向容器、文本、下拉框、复选框和按钮，页面滚动而底部操作区固定，按钮空间不足时换行。
常规页包含语言、自启开关和状态，并提供确定、取消、应用、恢复本页默认、重新加载及启动入口修复/重试。
...
布尔值与字符串共享编辑会话及条件保存；Application 另行注入 `startupApplied/startupStatus`，Settings
不依赖兄弟 SystemIntegration。同次语言与自启修改一次保存，之后分别核对持久化目标并应用副作用；
任一副作用失败保持待生效状态，不能宣称 JSON 与注册表之间具有原子事务。
...
App 先判定实例身份，再通过 Settings 的 `ReadStartupLanguage()` 只读用户语言及默认设置；
重复启动与参数错误提示也使用已保存语言，第二实例不创建设置或日志。正常主实例再完成设置初始化。
语言应用时刷新已存在的设置、欢迎、通用提示、隐藏窗口、截图窗口及托盘文字；底层捕获和输出诊断写日志，
展示使用本地化粗略错误，不将固定英文诊断拼入用户弹窗。系统对话框自身按钮语言仍由 Windows 决定。
...
`Application/SystemIntegration` 提供独立目标 `OpenST::SystemIntegration`，只依赖 Windows SDK 和标准库。
`StartupRegistration` 只操作 HKCU Run 的 `Open-ST` 值，命令为 `"EXE绝对路径" --startup`，不超过 260 字符；
查询区分缺失、当前路径、旧路径、外部冲突和失败。启停后重新读取核验，修复旧路径需业务明确授权。
不访问未公开的 StartupApproved 数据，也不把入口存在解释为 Windows 一定允许启动。

`SingleInstance` 使用包含当前用户 SID 的 Global 互斥体；命名管道拒绝远程连接、限制用户访问并校验对端、
会话和固定协议长度。异步接收线程只投递 App 消息，UI 线程按欢迎/忙状态执行或拒绝请求。
重复普通启动请求设置，`--capture` 请求截图，`--startup` 静默退出；管道未就绪时有限重试，不启动第二实例。
同用户跨登录会话不远程执行截图或窗口激活；退出事件取消未完成管道 IO，再释放线程与句柄。

首次欢迎属于 Settings，使用 Renderer 的内嵌 content 布局。确认前不创建启动项，取消退出不保存选择。
确认后先保存完成标记和自启意图，再应用系统入口；系统失败不撤销已经保存的选择，可重试或退出。
...
冻结指针、UIA 子控件命中与翻译尚未接入；OCR 已在当前开发树以可选功能接入。标注的文字、跨屏、属性与擦除、输出一致性四组实机检查已获用户确认通过；具体验收范围与未单独记录的平台组合见[实现进度](implementation_progress.md)。
...
- 单实例管道接收线程：校验固定启动命令后只投递 UI 消息，退出时取消 IO；不调用 Settings 或窗口业务。
...
- OCR 已在单工作线程执行，图像副本和任务由 Application/OCR 独占；合作取消立即撤销发布资格，实际后台结束前拒绝新任务。OcrSession 将不可变结果交给公共表单，主线程不等待模型初始化。
- 后续 HTTP 请求必须在后台执行，可取消；任何供应商回调都不能直接操作窗口。
- 设置写入使用临时文件加原子替换，避免崩溃留下半份 JSON。

OCR 具体并发和依赖设计见 [OCR 方案](design/ocr-v0.4.md)，图像准备仍在图形所属线程。不得将模型初始化或识别移入 UI。
...
`Application/Hotkeys` 管理进程内单调注册 ID、候选/活动/退役所有权及截图会话固定键匹配。
`app_hotkeys.cpp` 连接 Settings 与注册模块，保持 Settings/Renderer/Hotkeys 无兄弟依赖。
新组合先以独立 ID 注册，再由 Settings 一次条件提交全部修改；成功后无分配切换身份，失败撤销候选。
旧注册或候选注销失败仍可追踪重试，其消息永不分派；系统配置与进程崩溃不承诺原子事务。
通用 keyChord 控件处理录入、焦点和导航；业务允许范围、序列化和提示由 App 注入。
具体协议、作用域、失败恢复及人工验收见 [快捷键设计](design/hotkeys-v0.3.md)。

### 日志维护与全部默认恢复

Common 在保留当前输出流的前提下清理识别的历史文件，枚举不长时间持有写锁；删除时再次检查代次、当前文件及身份。
Application 独占一个可取消历史清理任务，持久保存结果后投递操作编号；主循环及设置重开可补收未送达结果。
后台不借用 Settings 或其 HWND；退出和退出清理先回收任务再关闭日志。Settings 只消费窄回调和 UI 状态。
立即恢复六项默认值复用候选草稿及条件提交，先准备快捷键、提交后激活，再应用语言/自启；默认值相同也复核运行状态。
详细边界及当前验证见 [日志与维护设计](design/log-maintenance-v0.3.md)。
