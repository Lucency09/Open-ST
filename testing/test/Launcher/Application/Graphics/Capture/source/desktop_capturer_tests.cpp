...
namespace
{
// gTest 的诊断流使用窄字符串；把 Win32 宽字符错误转成 UTF-8，才能保留中文失败原因。
...
// 集成 case 只有在真实显示器和交互式 input desktop 中才有意义。
// 这里仅判断平台前提；前提满足后的任何捕获错误都必须按失败处理。
...
// 检查原生 plane 是否至少含一个非黑 RGB 像素；alpha 不属于桌面颜色语义。
...
// 在具备显示器和交互式桌面的 Windows 会话中执行一次真实的虚拟桌面捕获。
// 验证冻结桌面和每个原生 plane 有效；HDR 输出若降级 BGRA8，必须显式记录系统 SDR 转换。
// 无显示器或无交互式桌面时属于平台条件不满足，测试会明确标记为 Skipped，而不是失败。
